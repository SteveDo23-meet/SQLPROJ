...
CREATE PROCEDURE CalculateCustomerLifetimeValue (IN custID INT, OUT totalSpent DECIMAL(10,2),
 OUT avgOrderValue DECIMAL(10,2), OUT orderCount INT)
BEGIN
...
SELECT 
    ...
    DATEDIFF(
        CASE WHEN MAX(o.OrderDate) IS NOT NULL THEN MAX(o.OrderDate) ELSE CURRENT_DATE END,
        CASE WHEN MIN(o.OrderDate) IS NOT NULL THEN MIN(o.OrderDate) ELSE CURRENT_DATE END
    ) AS DaysAsCustomer
FROM customers c
...
JOIN (
    SELECT CategoryID, AVG(Price) AS AvgCategoryPrice
    FROM products
    GROUP BY CategoryID
) cat_avg ON p.CategoryID = cat_avg.CategoryID
GROUP BY p.ProductID, p.ProductName, c.CategoryName, p.Price, cat_avg.AvgCategoryPrice
HAVING NumberOfOrders >= 3
   AND AvgRating > 4
   AND p.Price > cat_avg.AvgCategoryPrice
